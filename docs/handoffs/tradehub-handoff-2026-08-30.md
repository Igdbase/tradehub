...
- Replaced the long `/admin` surface with route-addressable focused Super Admin views for Overview, Workspaces, Licences, Payments, Integrations, Execution Safety, and Audit.
...
- Roadmap truthfully recorded: on 19 September 2026 the owner paused Stage 29H (Forex/MT5 Journal Sync) pending Stage 29G external acceptance, which remains deferred because no Google Cloud Secret Manager project or approved provider accounts exist. Stage 29N final freeze remains last and unstarted.
- Magnet snap: a per-session rail toggle with truthful on/off state. When on, anchors for point-based tools (Trend, Horizontal/Vertical lines, Fibonacci, Zone, Parallel Channel base) snap to the nearest revealed candle's Open/High/Low/Close within a bounded 12px radius; beyond it nothing snaps. Never touches hidden/future candles and never requests future data; saved geometry stores snapped values so reload reproduces exactly.
- Freehand Brush: press-drag-release captures one stroke, simplified to at most 120 points via distance-based simplification before commit, persisted as a versioned TradeHub-owned `tradehubFreehandBrush` overlay that reloads exactly and is selectable/deletable like other drawings; invalid or near-zero strokes persist nothing and restore Select.
- Parallel Channel: two anchors set the base trend line, a third interaction sets the parallel offset with translucent fill between the lines; both lines plus fill render from one persisted three-point record; Escape, pointer cancellation, tool switching, and navigation commit nothing and restore Select truthfully.
- All three tools are drawing-only and are covered atomically by Delete selected and Clear chart drawings; candles, orders, events, bookmarks, completion state, reports, journal records, and non-drawing annotations are untouched.
- Server validation enforces per-kind required point counts (brush 2..120, parallel channel exactly 3) while legacy kinds keep their existing rules; no new dependencies, providers, or network calls; KLineChart stays pinned at 10.0.3.
...
- Browser coverage adds a bounded Stage 30A test at laptop and tablet viewports for magnet truthfulness (snapped versus unsnapped), brush point bounds, channel persistence/reload, Escape cancellation, delete-selected, and atomic clear. Added `npm run stage30:qa` asserting the tools, bounds, boundaries, docs, and preserved frozen stage states.
- Practice remains student-owned, revealed-candle-only, and simulated-only, with no execution, AutoCopy, payment, provider, credential, or hidden-candle boundary change.
