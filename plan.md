...
External acceptance note on 6 September 2026: Stage 29G remains implemented/source-QA ready but owner-deferred for real Binance/Bybit provider acceptance because no Google Cloud Secret Manager project and no approved provider accounts are currently available. Do not mark Stage 29G owner/provider accepted, closed, or frozen until that checklist is completed.

### Stage 29H: Forex And MT5 Journal Sync

Status: unstarted and externally deferred.

- Stage 29H remains the future Journal-only Forex/MT5 history lane.
- It must not begin until Stage 29G external acceptance is either completed or explicitly paused by the owner.
...
- Real Paystack sandbox/production acceptance remains deferred in `manual-test-backlog.md` and is not claimed complete. Stage 29G external Binance/Bybit acceptance remains deferred. Stage 29H remains deferred/unstarted. Stage 29J is owner-accepted, closed, and frozen after successful local owner testing on 7 September 2026.

- Rebuilt `/app/copier` as a student-focused Trade Copier purchase and setup workspace instead of an internal operations dashboard.
- Adviser correction: `/app/copier` now consumes a dedicated allowlisted `StudentCopierOverviewResponse` from `/api/student/copier` rather than the broad `StudentCryptoExecutionOverviewResponse`.
- Student Copier checkout wrappers return only `{ ok, authorizationUrl }`; payment intent ids, references, access codes, amounts, currency, and payment metadata stay server-side.
- Privacy/regression correction: legacy `/api/student/crypto-execution/**` and `/api/student/forex-execution/**` student routes now either return the same allowlisted Copier DTO/minimal checkout response or an authenticated safe 410; raw `connectionId` mutation routes and live-production/canary student bypasses are retired.
- Opaque student action refs require `STUDENT_COPIER_ACTION_REF_SECRET` in production and fail closed when the configured secret is missing or weak; local deterministic fallback remains development-only.
- Owner unified-subscription correction: the unpaid state now shows one clear `Trade Copier` subscription card and one `Purchase Trade Copier` action. Trade Copier is a separate paid student add-on, is not included in Launch, Pro, or Enterprise workspace packages, and is one subscription rather than separate Crypto/Forex products.
- The page uses one authoritative canonical `trade_copier` billing lifecycle with dedicated Trade Copier Paystack configuration. One verified payment unlocks both `Crypto Setup` and `Forex Setup`; one cancellation locks both setup areas. Existing active legacy Crypto or Forex Copier subscriptions are materialized into one canonical entitlement without asking an already-paid student to pay again.
- Canonical protected checkout, verification, and cancellation now live under the student Copier route family and keep checkout responses minimized to `{ ok, authorizationUrl }`; legacy split purchase endpoints safely redirect through the unified lifecycle for compatibility.
- Final payment-lifecycle correction: cancellation authorization is separated from purchase eligibility, so a signed-in student who owns an existing canonical or grandfathered Trade Copier subscription can request cancellation even if course access, account mode, or eligibility changed after purchase. Cancellation locks Copier access immediately, schedules recoverable due Paystack disable work, invokes the same leased retry processor used by the Super Admin worker, marks cancellation complete only after provider confirmation or a matching disable webhook, and leaves failed/missing-provider cancellation in inactive support-safe `needs_attention`/retry states. Server-only subscription code/email token values are persisted after verification and never exposed through student DTOs.
- Bounded-cancellation consistency correction: repeated Cancel is now idempotent across `non_renewing` subscriptions with matching pending provider-disable work, valid `retry_scheduled` or `in_progress` tasks, `blocked` or `final_failed` support states, and already `resolved` provider-disable tasks. Matching work preserves its operation token, target, attempt count, due time, and lease; terminal support states are not reset; resolved work reconciles the canonical subscription to `cancelled` without another Paystack call; only a genuinely new Paystack subscription target after repurchase gets a fresh cancellation operation.
- Paystack cancellation retry is now operational through a Super Admin-owned worker route. Due retry tasks are queried oldest-first with a strict bound, initial cancellation never writes an unleased `in_progress` task, every `in_progress` task has an owner token and lease expiry, provider calls are rechecked immediately before disablement, abandoned leases recover safely, repeated cancellation preserves attempt counts, future tasks cannot starve due work, and stale work never targets a newer active subscription.
- Trade Copier Paystack webhooks are product-routed before the course/package webhook updater. Canonical Trade Copier charge, renewal charge with a new transaction reference, invoice-failure, non-renewing, and disable events update only the `trade_copier` lifecycle by proven product metadata, stored payment reference, or stored subscription code; course/package events do not affect Trade Copier and Trade Copier events do not affect course/package subscriptions. Webhook helpers now return truthful applied outcomes so stale, mismatched, rejected, or no-op events are recorded without claiming they processed a lifecycle transition.
- `subscription.not_renew` records an honest non-renewing/cancellation-pending lifecycle rather than falsely marking the paid period expired. Copier setup remains locked after student cancellation while stored provider status stays truthful until `subscription.disable` or local/manual completion finalizes cancellation.
- Recurring renewals validate against the current stored Trade Copier subscription code, product, Paystack plan code, amount, and currency; they use only authoritative `next_payment_date`/`period_end` values for the paid period. Missing or inconsistent renewal payloads preserve the existing paid period and record bounded support review instead of inventing an expiry from `paid_at`, `created_at`, or environment-only plan changes.
- Checkout initialization, verification callbacks, legacy callbacks, webhook status updates, renewals, and provider-cancellation completion now re-read the current canonical lifecycle revision, operation token, and latest intent/target before writing, so stale asynchronous completions cannot overwrite cancellation or a newer checkout. Replayed verification after cancellation returns a truthful support-review state and cannot claim setup availability or emit a misleading payment-verified transition.
- The shared server-only billing resolver now gates Crypto setup/routing/sandbox/production/canary checks and Forex provisioning/demo/canary checks; billing authorization alone still does not bypass consent, risk, provider permission, pause, sandbox, kill-switch, vault, or execution gates.
- Added emulator-backed `stage29i:repository:qa` coverage for canonical checkout, pending state, successful/replayed verification, product/amount/currency mismatch failures, legacy Crypto-only/Forex-only/both-active grandfathering, canonical-cancelled overriding stale legacy active records, cancellation after eligibility loss, provider-confirmed cancellation, provider cancellation failure/retry, repeated cancellation idempotency across pending/in-progress/blocked/final-failed/resolved provider-disable work, authoritative post-retry cancellation audit status, initial interruption recovery, due-task starvation prevention, leased `in_progress` tasks, abandoned retry leases, concurrent retry workers, missing subscription code/token, stale verify/cancel and checkout/cancel races, recurring renewals by subscription code, missing renewal period review, stored-plan validation, non-renewing/disable task resolution, webhook product isolation, truthful stale webhook receipts, duplicate/out-of-order webhooks, retryable cleanup, and concurrent checkout races.
- Entitled students see a compact setup choice between `Crypto Setup` and `Forex Setup` rather than every setup, preview, and ops panel at once.
- Crypto Setup keeps Binance/Bybit account setup, read-only student-relevant connection health, consent, risk limits, pause/resume, and disable/refresh controls through the dedicated protected Copier APIs with opaque student action refs.
- Forex Setup keeps MT4/MT5 broker setup, payment, consent, risk limits, pause/resume, and disable controls while mapping internal statuses to the closed student labels `Setup received`, `Ready for review`, `Connected`, `Needs attention`, `Disabled`, and `Unavailable`.
- Internal operational UI and copy such as stages, workers, vaults, canaries, raw readiness gates, provider payloads, fingerprints, payment intent internals, infrastructure diagnostics, and support internals are absent from the normal student Copier page.
- Journal Sync remains separate from Copier and available without a Copier purchase; `/app/copier` does not request Journal Sync APIs.
- Browser coverage now checks unpaid purchase CTA, deterministic checkout initiation/callback handling, consent/risk saves across reload, pause/reload/resume/reload, connection disablement, Forex setup disablement, legacy overview/raw-route privacy, wrong-role route/API denial, populated Forex operational-source sanitization, Journal Sync independence, forbidden internal terminology, no public package prices, and laptop/tablet/narrow responsive widths.
- Security boundaries remain unchanged: no provider calls, credentials, payment products, entitlement shortcuts, real orders, live execution, AutoCopy gate weakening, raw ids, vault refs, provider payloads, or diagnostics were added or exposed.
...
- Rebuilt `/app/signals` as a compact student trading feed with `All`, `Forex`, `Crypto`, and `Open` filters, dense TradeHub-owned rows, symbol identity, Buy/Sell badges, entry, SL, TP, age, lifecycle status, provider-confirmed copied/executed state, and supported P&L only when authoritative linked execution data exists.
- Added a dedicated allowlisted student Signals DTO and server repository. Reads are authenticated, student/workspace scoped, newest-first, bounded, and return truthful pagination/truncation metadata without exposing workspace ids, student ids, connection ids, intent ids, provider ids, credentials, payloads, diagnostics, risk internals, execution internals, or source record ids.
- The student feed includes explicit direct `in_app` TradeHub influencer signals, with source-less legacy TradeHub signals preserved only as display-only legacy records. Unknown, Telegram-like, webhook-like, future-source, malformed, draft, and unsupported-status records remain hidden and non-executable until a later approved stage.
- Copied or Executed labels are shown only for provider-confirmed linked account-ledger executions with server-only `tradeHubSignalId` linkage, copied trade origin, supported real execution mode, matching market, and normalized matching symbol. Queued, eligible, attempted, paper, demo, testnet, dry-run, failed, blocked, unconfirmed, wrong-market, and unsupported rows do not appear as copied/executed and do not fabricate P&L.
- Signal feed pagination now uses a bounded newest-first cohort with a stable tie-breaker, opaque cursor metadata, and a visible Load More action only when another page exists inside the loaded cohort. `truncated` is separate and means the repository hit its scan boundary; the UI shows a restrained bounded-history notice when older records may be omitted.
- Signal P&L now uses a typed canonical `authoritativePnl` shape and is omitted unless authoritative floating or realized value and currency are both available; missing currency no longer defaults to USD and ad hoc test-only P&L fields are not accepted by the feed.
- Hardened existing Crypto and Forex signal routing entry points so only direct published `in_app` signals can reach the existing routing modules. Crypto signals remain crypto-only, Forex signals remain forex-only, and existing Trade Copier billing, market setup, consent, risk, connection, freshness, pause, kill-switch, idempotency, reconciliation, and live-execution gates remain in force.
- Adviser correction: Crypto routing now enforces the canonical Stage 29I Trade Copier billing resolver during production routing and honors the student kill-switch before creating executable intents.
- Adviser correction: successful provider-confirmed Forex live-canary fills now write idempotent canonical account-linked ledger entries through the shared mapper/writer with server-only TradeHub signal linkage, copied origin, matching Forex market/symbol, provider confirmation, supported live-canary execution mode, and open/partial lifecycle. Dry-run, submitted, failed, rejected, blocked, or unconfirmed Forex attempts do not create copied/executed ledger evidence.
- Adviser correction: Crypto production and Forex live-canary workers now reload the authoritative signal after claiming an intent and before credential/token loading or provider calls. They require the same workspace, signal id, explicit direct `in_app` source, published status, original market, normalized symbol/pair, direction, and routed signal version, then recheck current student billing, consent, pause/revoke/kill-switch, connection, platform, and workspace gates at the final execution boundary.
- Adviser correction: provider-confirmed Crypto and Forex results now persist terminal attempt truth before canonical ledger projection. Ledger projection failure records indexed due-work status, owner-token/fencing repair lease fields, bounded retry timing/attempts, explicit skipped/not-applicable/final-failed outcomes, and final-failed support state. Repair queries actionable due work and expired in-progress work through separate bounded indexed queries, merges them by one effective eligible timestamp, tie-breaks by `updatedAt` and document id, then applies the batch limit so neither category can starve the other. Repair reloads each attempt before every projection-state write, updates only when the stored owner token still matches, runs independently before live/canary/order-call/vault/provider gate returns, loads no credentials, calls no provider, and can reconcile a deterministic ledger row created by a stale worker without a second provider request or duplicate ledger row. Poisoned final-failed projection work does not starve later due records and stale workers cannot clear another worker's lease or regress projected state.
- Adviser correction: production routing exports reject actor/signal workspace mismatches before any intent, attempt, audit, response, or provider decision can be written from a cross-workspace actor.
- Added deterministic browser fixtures and student E2E coverage for direct signals, unknown/external source exclusion, equal timestamps, more than 250 mixed records, bounded-history notice, Load More, All/Forex/Crypto/Open filters, copied/executed truthfulness, unsupported P&L absence, wrong-role API/page denial, response privacy, responsive widths, and cleanup.
- Added `npm run stage29j:qa` source QA plus `npm run stage29j:routing:qa` executable Firestore-emulator coverage that invokes the actual Crypto production routing path, Forex live-canary routing/worker path, shared ledger mappers/writers, student feed repository, pagination, idempotency, blocked-case gates, route-then-cancel/edit/pause/revoke/kill-switch races, provider-success/ledger-projection-failure repair, indexed due-work discovery beyond projected prefixes, disabled-gate repair independence, bounded final-failure, fenced late-success/late-failure repair races, active-lease starvation prevention, fair due-versus-expired scheduling, claim-time final-failed counting, truthful repair counters, and workspace-mismatch rejection with deterministic injected providers.
- Owner acceptance closure: the compact Signals feed, `All`/`Forex`/`Crypto`/`Open` filtering, provider-confirmed copied/executed badges, P&L omission rules, direct `in_app` routing gates, projection repair posture, and responsive student experience were owner-accepted on 7 September 2026. Stage 29J is closed and frozen unless a new reproducible defect is reported.
- Stage 29I remains closed and frozen. Stage 29G external Binance/Bybit acceptance remains deferred. Stage 29H remains deferred/unstarted. Stage 29K is owner-accepted, closed, and frozen below. Stage 29L is owner-accepted, closed, and frozen after successful local owner testing on 8 September 2026. Stage 29M Super Admin Navigation And Demo Account Reliability is implemented/source-QA ready below, and Stage 29N remains unstarted and next.
...
- Added a default-disabled, server-only Telegram webhook foundation for approved source ingestion. The route accepts POST only, validates Telegram's secret-token header, enforces bounded JSON payload size/content type, rejects stale/unsupported updates, uses keyed opaque Telegram source/message identities, and stores no raw Telegram chat ids, usernames, message bodies, payloads, tokens, webhook secrets, or provider diagnostics.
- Telegram updates can create only quarantined external signal candidates through the existing Stage 24 parser/review contract. Source allowlist matching is explicit, workspace-scoped, and fail-closed; duplicate delivery is idempotent and does not create duplicate candidates.
- Added a controlled workspace bridge that promotes only `approved_for_workspace_preview` Telegram candidates into canonical workspace signals with safe moderation proof metadata. Promotion is idempotent, proof-bearing, and routes only through the existing Stage 29J Crypto/Forex routing paths so Trade Copier billing, market setup, consent, risk, freshness, pause, kill-switch, idempotency, projection repair, and live-execution gates remain authoritative.
- Student Signals remain allowlisted and privacy-safe. Direct in-app TradeHub signals still display normally; Telegram signals can appear only after moderation/promotion proof exists; unknown, malformed, webhook-like, future-source, stale, draft, unsupported-market, and unapproved external records stay hidden and non-executable.
- Crypto production and Forex live-canary workers now revalidate the moderated Telegram source allowlist after claiming an intent and before credential/token loading or provider calls. Disabled, mismatched, removed, or stale source proof terminates safely without provider requests.
- Stage 29K adviser correction added deterministic Telegram source identity records, workspace-rebind rejection, immutable moderation snapshot checks, exact source-record/version attestation, malformed timestamp quarantine, and destination-aware routing outbox checkpoints for Crypto Paper, Crypto Sandbox, Crypto Production, Forex Paper, Forex Demo, and Forex Live Canary.
- Stage 29K owner-visible setup correction makes the Super Admin Telegram source form functional end-to-end: choosing Telegram submits the required Telegram parser mode server-side without exposing parser controls, requires workspace/status/label/symbol/market inputs, and browser coverage creates the source through the visible form instead of bypassing it with a direct Admin API request.
...
- Routing destination state is independently bounded per destination. Already completed destinations are skipped, retryable destination failures pause the current pass, final-failed destinations do not block later destinations, and the outbox becomes `completed_with_failures` only after every applicable destination is either completed or final-failed.
- Infrastructure dispatch failures before destination routing now use a separate bounded `dispatchAttempts` counter. Attempts 1-4 persist and report `retry_scheduled`; attempt 5 persists and reports `failed`, clears retry scheduling, and is no longer rediscovered by later workers.
- Added Firestore indexes for Telegram source identity lookup, candidate delivery deduplication, and routing outbox due/expired lease lookup while keeping Firestore browser rules deny-by-default for external ingestion storage.
- Added `npm run stage29k:qa`, `npm run stage29k:ingestion:qa`, and `npm run stage29k:closure:qa` for source-level, Firestore-emulator, and owner-acceptance closure coverage of default-disabled gates, source allowlisting, stale/unknown update rejection, malformed timestamp handling, duplicate delivery, candidate quarantine, approved preview promotion, stale-publication rejection, destination-specific routing repair, independent destination retry exhaustion, infrastructure exhaustion truthfulness, final source revalidation, privacy boundaries, and closure status.
...
- Replaced the long `/workspace` experience with route-addressable focused workspace views for Home, Students, Signals, Courses, Practice, Copier, Billing, Branding, and Enterprise.
- Added a shared workspace navigation shell with active-route state, direct URLs, refresh/deep-link support, and compact horizontal navigation that works across desktop, tablet, and narrow widths without nesting another app shell.
- `/workspace` is now a summary and next-actions home only. It no longer renders detailed student tables, signal forms, Telegram preview publication, course controls, practice assignment tools, billing operations, branding/domain details, or Enterprise forms.
- Focused route responsibilities are preserved: Students owns student management and support actions; Signals owns direct signal management plus approved external-preview publication; Courses owns course visibility and Course Hub navigation; Practice owns aggregate insights, assignments, cohorts, and feedback; Copier owns workspace-safe Trade Copier readiness; Billing owns licence/package/billing status; Branding owns branding/domain readiness; Enterprise owns deployment/SLA posture and Enterprise integration requests.
- Scoped client loading now fetches dashboard essentials first, then loads only the active view's protected workspace endpoints. A failed section request stays local to that focused view and does not break navigation or unrelated routes.
- Adviser correction tightened scoped post-mutation refreshes: creating/publishing a Signal refreshes Signals, external-preview readiness, Copier readiness, and dashboard only; updating Student support refreshes Students and dashboard only. These mutations no longer fetch hidden Practice endpoints or Billing overview data.
- Responsive navigation correction keeps the active focused-route item visibly inside the bounded nav viewport at desktop, tablet, and mobile widths, including direct mobile loads of `/workspace/enterprise` and `/workspace/branding`, without introducing document-level horizontal overflow.
- Workspace-facing copy was tightened to avoid stale stage/build terminology and unnecessary private-system wording while preserving the existing protected server checks.
- The full Course Hub remains available at `/workspace/courses/hub`, and existing individual course editor routes remain under `/workspace/courses/[courseId]`.
- Browser coverage now checks all nine workspace destinations, active nav state, visible active nav geometry, direct URL loading, Back/Forward behavior, home summary-only behavior, scoped endpoint loading, Student and Signal post-mutation request isolation, focused section reachability, Stage 29K publication from `/workspace/signals`, wrong-role blocking, response privacy, and forbidden private/implementation text.
...
- Added `npm run stage29l:qa` and `npm run stage29l:closure:qa` to verify the route files, shared focused navigation, scoped loading, post-mutation isolation, responsive nav coverage, source-copy boundaries, browser coverage, documentation handoff, owner-accepted closure status, and frozen Stage 29K/29J/29I/29F QA wiring.
- Stage 29K is owner-accepted, closed, and frozen. Stage 29J remains owner-accepted, closed, and frozen. Stage 29I remains closed and frozen. Stage 29F remains owner-accepted, closed, and frozen. Stage 29G external Binance/Bybit acceptance remains deferred. Stage 29H remains deferred/unstarted. Stage 29M is implemented/source-QA ready below, and Stage 29N remains unstarted and next.
...
- Replaced the long `/admin` surface with focused Super Admin routes for Overview, Workspaces, Licences, Payments, Integrations, Execution Safety, and Audit.
- Default Overview is now high-level and action-oriented. It shows safe source/status summary, KPI summary, and aggregate operator issue posture without rendering every application, payment, integration, execution, or audit panel.
- Added a shared Super Admin navigation shell with direct route URLs, active-route state, Back/Forward behavior, and bounded horizontal navigation that keeps the active item visible at desktop, tablet, and mobile widths.
- Scoped loading now fetches only what the active admin view needs: Workspaces loads applications, Payments loads payment overview, Integrations loads branding/Enterprise plus dormant messaging and external signal ingestion, Execution Safety loads AutoCopy readiness, and Audit loads trust/safety plus audit history.
- Dangerous execution/canary/reconciliation controls are isolated under `/admin/execution-safety` instead of appearing in routine Overview.
- Messaging remains preserved as a dormant no-send compatibility/readiness panel inside Integrations, but it is not a normal top-level admin navigation destination.
- Demo account reliability remains deterministic through the Stage 28A seed pack and browser auth helpers for `demo.superadmin@example.test`, workspace/influencer personas, and student personas.
- Browser coverage now checks all focused Admin routes, active nav state, visible active nav geometry, direct URL loading, browser history, scoped admin endpoint loading, concise Overview behavior, wrong-role blocking, response privacy, document overflow, and Stage 29K visible Telegram-source setup from `/admin/integrations`.
- Added `npm run stage29m:qa` to verify focused Admin routes, scoped loading, no load-everything helper, no Messaging top-level nav, deterministic seeded personas, docs, and frozen Stage 29L/29K/29J/29I/29F closure states.
- Stage 29M adviser correction (done and verified; status remains implemented/source-QA ready, owner acceptance is not claimed):
  - Truthful Admin Overview: AdminSupportOverview renders a non-numeric "—" state with "not measured on this view — open Payments" / "not measured on this view — open Execution Safety" detail for chips whose backing data was not loaded; the hardcoded MVP browser QA constant and chip were removed; no numeric zero is shown for unloaded data; pending apps and workspace readiness stay numeric from the overview payload; the aggregate-only privacy copy is intact; no new API fetches were added to the Overview view.
  - Genuinely scoped loading: `/admin/workspaces` no longer fetches `/api/admin/overview`; the dedicated applications effect (with a refresh nonce in its deps) is the single owner of initial and filter application loading, so exactly one `GET /api/admin/applications` fires on mount; the Refresh button and `refreshAll` reload the list via the nonce; application saves and workspace-shell creation refresh only the applications list (the audit event is already inserted locally); no `/api/admin/overview` request fires from workspaces load, filter change, save, or workspace-shell creation; licences, payments, integrations, audit, and execution-safety scoped loading and post-mutation refreshes are unchanged.
  - Browser coverage: a bounded single test signs in all seven seeded personas through the real `/login` form and asserts role-correct landings — superadmin to `/admin`, all three influencer personas to `/workspace` and blocked from `/admin`, students to `/app` with their truthful rendered surface (Active; the pending seed's not-active subscription renders Unknown access with locked surfaces; Past Due) — signing out between personas.
  - `npm run stage29m:qa` source assertions now cover all of the above, the browser spec's `/admin/workspaces` expectation excluding `/api/admin/overview`, and docs naming all seven persona emails.
  - Adviser verification on 18 September 2026: `stage29m:qa`, `typecheck`, `lint`, and `build` passed, and the 22-test `browser:qa:workspace-admin` Chromium suite passed locally against the Firebase emulators after `npm run seed:demo`. Known observation recorded, not fixed: the pending seed's `payment_pending` status is not a recognized student subscription access label, so `/app` truthfully renders "Unknown" access; renaming it is a product change outside this correction.
...
### Stage 30A: Practice Terminal Chart Tools Expansion (Magnet, Brush, Parallel Channel)

Status: implemented/source-QA ready. Owner acceptance is not claimed.

- The owner paused Stage 29H (Forex/MT5 Journal Sync) on 19 September 2026 pending Stage 29G external acceptance, which remains deferred because no Google Cloud Secret Manager project or approved provider accounts exist. Stage 29N final freeze remains last and unstarted.
- Added Magnet snap as a per-session rail toggle with visible on/off state and an accessible label. When on, pointer anchors for point-based tools (Trend, Horizontal/Vertical lines, Fibonacci, Zone, Parallel Channel base) snap to the nearest revealed candle's Open/High/Low/Close within a bounded 12px radius: time snaps to the candle and price snaps to the candle value. Beyond the radius nothing snaps, and hidden/future candles are never read or requested. Saved geometry stores the snapped values, so reload reproduces exactly.
- Added the Freehand Brush tool: press-drag-release captures one pointer stroke, simplifies it to at most 120 points via distance-based simplification before commit, and persists it as a versioned TradeHub-owned `tradehubFreehandBrush` overlay that reloads exactly and is selectable and deletable like other drawings. Invalid or near-zero strokes persist nothing and restore Select.
- Added the Parallel Channel tool: two anchors define the base trend line, then a third interaction (pointer move then click) sets the offset for the parallel copy with a translucent fill between the lines, mirroring the bounded-Fibonacci interaction style. Both lines plus the fill render from one persisted three-point record; Escape, pointer cancellation, tool switching, and navigation commit nothing and restore Select truthfully.
- All three tools are drawing-only: they never touch candles, orders, events, bookmarks, completion state, reports, journal records, or non-drawing annotations, and Delete selected plus Clear chart drawings cover them atomically like every other drawing.
- Server validation now enforces per-kind required point counts: brush strokes persist 2..120 chart points and parallel channels exactly 3, while every legacy kind keeps its existing count rule and storage bound.
- No new dependencies, providers, or network calls were added; KLineChart stays pinned at 10.0.3 and the overlays remain TradeHub-owned like the existing bounded Fibonacci.
- Frozen-guard reconciliation: because Stage 30A activates the formerly disabled Brush and Magnet snap placeholders, the stale "coming soon" placeholder assertions were corrected (not weakened) in the student browser suite and the Stage 29D.5/29D.6 QA guards (the still-unbuilt line variants Ray, Extended line, Horizontal ray, and Cross line remain asserted as disabled). Separately and independently of Stage 30A, `stage29d9:qa` was verified to already fail at the pre-Stage-30A HEAD because multiple of its assertions still reference the pre-KLineChart drawing implementation; repairing it is out of Stage 30A scope and is recorded here as a known broken legacy guard. `stage29d16:qa` and all other current Practice drawing guards remain unchanged and green.
- Practice browser coverage now includes a bounded Stage 30A test at laptop and tablet viewports proving magnet snap truthfulness (snapped versus unsnapped coordinates), the brush point bound, channel persistence/reload, invalid brush handling, Escape cancellation, delete-selected, and atomic clear.
- Added `npm run stage30:qa` asserting the three rail tools, bounded magnet snap logic, brush simplification bound, channel persistence shape, drawing-only boundaries, no future-data requests, updated docs, and preserved frozen stage states. `stage29d16:qa` and the existing Practice drawing suites remain the frozen-behavior guards.
- Practice remains student-owned, revealed-candle-only, and simulated-only. No execution, AutoCopy, payment, provider, credential, or hidden-candle boundary changed.
