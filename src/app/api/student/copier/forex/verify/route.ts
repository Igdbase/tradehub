...
function parseReference(request: Request) {
  ...
  if (!reference || reference.length > 160) {
    throw new AdminApiError(400, "invalid_reference", "A valid Forex Copier checkout reference is required.");
  }
...
export async function GET(request: Request) {
  try {
    ...
    const result = await verifyLegacyTradeCopierCheckout(actor, parseReference(request), "forex_autocopy");
