...
import { verifyStudentTradeCopierCheckout } from "@/lib/student-copier/student-copier-billing";
...
function parseReference(request: Request) {
  ...
  if (!reference || reference.length > 160) {
    throw new AdminApiError(400, "invalid_reference", "A valid Trade Copier checkout reference is required.");
  }
...
export async function GET(request: Request) {
  try {
    ...
    const result = await verifyStudentTradeCopierCheckout(actor, parseReference(request));
